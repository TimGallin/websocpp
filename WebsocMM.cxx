...
namespace WebsocMMM{
	...
	void WebsocMM::Run(){
		...
		for(;;) {
			r = RawRead(&_rxbuf[0] + r, _rxbuf.size());
			...
			if (r <= 0) {
				...
				if (!recv_parse_handle(_rxbuf, r)){
					_rxbuf.resize(_rxbuf.size() + r);
				}
			}
		}
	...
	*/
	...
	bool WebsocMM::connect(){
		...
		if (sscanf_s(line, "HTTP/1.1 %d", &status) != 1 || status != 101) {
			return false;
		}
